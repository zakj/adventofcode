...
main(
  (s) => shuffle(makeDeck(10007), parse(lines(s))).findIndex((c) => c === 2019),
  (s) => {
    const deckSize = 119315717514047n;
    const shuffleTimes = 101741582076661n;
    const { offset, increment } = shuffle2(deckSize, parse(lines(s)));
    ...
    const offsetIter =
      ...
      deckSize;
    return Number((offsetIter + 2020n * incrementIter) % deckSize);
  }
);
